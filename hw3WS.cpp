...
int main() {
	...
	if (pid == 0)
	{
		...
		//	sendrequest(readrequest(request1), request2);
			...
			if (ws[pi].size()<r)
			{
				ws[pi].push_back(addr.substr(0,segmentnumber + pagenumber));
				...
				for (int x = 0; x < siz; x++)
				{
					if (ws2[pi].front() == addr.substr(0, segmentnumber + pagenumber))
					{
						inws = true;
						break;
					}
					ws2[pi].push_back(ws2[pi].front());
					ws2[pi].pop_front();
				}
				...
				if (!inws)
				{
					...
					ws2[pi].push_back(addr.substr(0, segmentnumber + pagenumber));
			...
			{
				string poped = ws[pi].front();
				ws[pi].pop_front();
				ws[pi].push_back(addr.substr(0, segmentnumber + pagenumber));
				//check if still in ws
				...
				for (int x = 0; x < siz; x++)
				{
					if (ws[pi].front() == poped)
					{
						inws1 = true;
						break;
					}
					ws[pi].push_back(ws[pi].front());
					ws[pi].pop_front();
				}

				if (!inws1)
				{
					int segi2 = bintodec(atoi(poped.substr(0, segmentnumber).c_str()));
					int pagi2 = bintodec(atoi(poped.substr(segmentnumber, pagenumber).c_str()));

					int framereplace = pagetable[pi][segi2][pagi2];

					pagetable[pi][segi2][pagi2] = -1;

					framesavailable.push_back(framereplace);
					//write request
				...
				//check if new request is in ws2
				bool inws2 = false;
