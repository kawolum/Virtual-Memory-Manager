		...
		//	sendrequest(readrequest(request1), request2);
			...
			if (!inframes)
			{
				...
				//	cout << "hf" << endl;
					...
					opt[pi].push_back(addr.substr(0, segmentnumber + pagenumber));
			...
			//		cout << pagetable[pi][segi][pagi] << endl;
				...
				//	cout << "pr" << endl;
					...
					copy(pidr.begin(), pidr.end(), back_inserter(pidr2));
					copy(addrr.begin(), addrr.end(), back_inserter(addrr2));
					...
					for (int x = 0; x < r; x++)
					{
						for (int y = 0; y < siz; y++)
						{
							if (pidr2.front() == pid)
							{
								if (opta[x] == addrr2.front().substr(0, segmentnumber + pagenumber))
								{
									break;
								}
								optcount[x] += 1;
							}
							pidr2.push_back(pidr2.front());
							pidr2.pop_front();
							addrr2.push_back(addrr2.front());
							addrr2.pop_front();

						}
					}

					int highestcount = optcount[0];
					int highestcountindex = 0;

					for (int x = 1; x < r; x++)
					{
						if (optcount[x]>highestcount)
						{
							highestcount = optcount[x];
							highestcountindex = x;

						}
					}
					//cout << opta[highestcountindex2]<<"{" << endl;

					int segi2 = bintodec(atoi(opta[highestcountindex].substr(0, segmentnumber).c_str()));
					int pagi2 = bintodec(atoi(opta[highestcountindex].substr(segmentnumber, pagenumber).c_str()));
					opt[pi].remove(opta[highestcountindex]);
