		...
		//	sendrequest(readrequest(request1), request2);
			...
			if (!inframes)
			{
				...
				//	cout << "hf" << endl;
					...
					for (int x = 0; x < r; x++)
					{
						if (mfu[pi][x] != "-1")
						{
							mfu[pi][x] = addr.substr(0, segmentnumber + pagenumber);
							count[pi][x] =1;
							mfusize[pi]+=1;
							break;
						}
			...
			//		cout << pagetable[pi][segi][pagi] << endl;
				...
				//	cout << "pr" << endl;
					...
					int highestcount = count[pi][0];
					int highestcountindex = 0;

					for (int x = 1; x < r; x++)
					{
						if (count[pi][x] > highestcount)
						{
							highestcount = count[pi][x];
							highestcountindex = x;
							break;
						}
					}

					int segi2 = bintodec(atoi(mfu[pi][highestcountindex].substr(0, segmentnumber).c_str()));
					int pagi2 = bintodec(atoi(mfu[pi][highestcountindex].substr(segmentnumber, pagenumber).c_str()));
					...
					mfu[pi][highestcountindex] = addr.substr(0, segmentnumber + pagenumber);
					count[pi][highestcountindex] = 1;
				//	cout << addr.substr(0, segmentnumber + pagenumber) << endl;
			...
			//	cout << "in frame" << endl;
				...
				for (int x = 0; x < r;x++)
				{
					if (mfu[pi][x] == addr.substr(0, segmentnumber + pagenumber))
					{
						count[pi][x] +=1;
					}
				}
